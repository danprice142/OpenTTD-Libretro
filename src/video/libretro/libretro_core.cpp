...
/* VFS interface */
static struct retro_vfs_interface *vfs_interface = nullptr;

/* Frame pumping during long operations */
static void pump_libretro_frame();
static uint32_t loading_screen_buffer[1280 * 720];
...
static void pump_libretro_frame()
{
	/* During world generation, pump frames to keep RetroArch responsive.
	 * This is called frequently during progress updates, so we throttle
	 * to avoid excessive rendering overhead (only update ~10 times per second). */
	if (!video_cb || !audio_batch_cb) return;

	/* Throttle updates to avoid slowing down world generation */
	static auto last_pump = std::chrono::steady_clock::now();
	auto now = std::chrono::steady_clock::now();
	auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - last_pump);

	/* Only pump frames every 100ms (10 FPS during generation) */
	...
	if (driver && openttd_initialized) {
		/* Update windows to refresh progress dialog */
		UpdateWindows();

		/* Paint the progress window to the video buffer */
		driver->PaintProgressFrame();

		/* Send the rendered frame to frontend */
		...
		if (buffer) {
			...
			memset(loading_screen_buffer, 0x20, sizeof(loading_screen_buffer));
			video_cb(loading_screen_buffer, video_width, video_height, video_width * 4);
		}
	} else {
		/* Show loading screen during initial startup */
		memset(loading_screen_buffer, 0x30, sizeof(loading_screen_buffer));
		video_cb(loading_screen_buffer, video_width, video_height, video_width * 4);
	}
...
static bool initialize_openttd()
{
	...
	/* Ensure map size settings are valid (minimum 6 = 64 tiles per map_type.h) */
	extern GameSettings _settings_game;
	extern GameSettings _settings_newgame;
	if (_settings_game.game_creation.map_x < 6) _settings_game.game_creation.map_x = 6;
	if (_settings_game.game_creation.map_y < 6) _settings_game.game_creation.map_y = 6;
	if (_settings_newgame.game_creation.map_x < 6) _settings_newgame.game_creation.map_x = 6;
	if (_settings_newgame.game_creation.map_y < 6) _settings_newgame.game_creation.map_y = 6;

	/* Copy newgame settings to game settings (like PC version does before game start) */
...
RETRO_API void retro_set_environment(retro_environment_t cb)
{
	...
	/* Request VFS interface from frontend */
	struct retro_vfs_interface_info vfs_info = { 1, nullptr };
	cb(RETRO_ENVIRONMENT_GET_VFS_INTERFACE, &vfs_info);
	if (vfs_info.iface) {
...
struct retro_vfs_interface *GetVFS() { return vfs_interface; }
bool HasVFS() { return vfs_interface != nullptr; }

} /* namespace LibretroCore */

void LibretroCore_PumpFrame()
{
	pump_libretro_frame();
}
